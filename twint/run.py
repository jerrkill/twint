...
class Twint:
    def __init__(self, config):
        ...
        self.token = token.Token(self.config.Bearer_token)
        self.config.Guest_token = self.config.Guest_token if self.config.Guest_token is not None else self.token.refresh()
    ...
    async def Feed(self):
        ...
        while True:
            ...
            try:
                ...
                self.config.Guest_token = self.token.refresh()
                response = await get.RequestUrl(self.config, self.init)
    ...
    async def profile(self):
        ...
        for tweet in self.feed:
            self.count += 1
            tmptws = await output.Tweets(tweet, self.config, self.conn)
            global g_tweets
